...
def clean (filename):
    ...
    newFileName = newFileName.replace("(Sp)", "")  
    newFileName = newFileName.replace("ebook", "")
    newFileName = newFileName.replace("revista", "")
    newFileName = newFileName.replace("andreas faber kaiser", "")
    newFileName = newFileName.replace("españa", "")
    newFileName = newFileName.replace("espana", "")
    newFileName = newFileName.replace("Espana", "")
    newFileName = newFileName.replace("motor16", "")
    
    newFileName = newFileName.replace("es downmagaz net", "")
    newFileName = newFileName.replace("es downmagaz com", "")
    newFileName = newFileName.replace("Varios Autores", "")
    newFileName = newFileName.replace("Issue", "")
    newFileName = newFileName.replace("de ciencia ficcion (kvflr)", "")
    newFileName = newFileName.replace("Wd", "")
...
def seekEpubConverts(token,newFileName):
    ...
    if token == "National Geographic" : maxsize  = 180
    elif token == "White Dwarf" : maxsize  = 140
    elif token == "Coches Clasicos" : maxsize  = 118
    elif token == "Motor Clasico" : maxsize  = 124
    else:
       maxsize  = 100
    try:
...
def findYears (years,fulldate,revista): 
    for y in range(yearsRange):
        if revista.find(str(initialYear+y)) > -1 : 
            years[y] += 1
...
def printYears(years):
    ...
    for i in range(len(years)):
        ...
        if yearsIndex > 9 : 
            ...
            print(yearsGrid)
            yearsGrid = "year " + "\t"
            yearsGrid += str(initialYear+i) + ":" + totYears + "  "  # +"avg " + "{:.1f}".format( years[i]/12) + " " 
        else :
            yearsGrid += str(initialYear+i) + ":" + totYears + "  "  #+ "avg " + "{:.1f}".format( years[i]/12) + " " 
